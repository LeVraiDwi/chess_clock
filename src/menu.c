...
void	ft_creat_custom(s_game *game, GtkWidget *grid)
{
	...
	//temps joueur premier
	button = 0;
	button = gtk_label_new("temps du premier joueur:");
	context = gtk_widget_get_style_context(button);
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_plus_min_j1), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 6, 1, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("-");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_minus_min_j1), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 6, 3, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("+");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_plus_second_j1), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 8, 1, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("-");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_minus_second_j1), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 8, 3, 1, 1);
	//creation des label
	game->label_custom[2] = gtk_label_new("00");
	...
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_label");
	gtk_grid_attach(GTK_GRID(grid), game->label_custom[2], 9, 2, 1, 1);
	game->label_custom[3] = gtk_label_new("00");
	context = gtk_widget_get_style_context(game->label_custom[3]);
	gtk_style_context_add_class(context, "mod_label");
	gtk_grid_attach(GTK_GRID(grid), game->label_custom[3], 11, 2, 1, 1);
	gtk_grid_attach(GTK_GRID(grid), button, 10, 2, 1, 1);
	//temps joueur deux
	button = 0;
	button = gtk_label_new("temps du deuxieme joueur:");
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_stitre");
	gtk_label_set_xalign(GTK_LABEL(button), 0.7);
	gtk_grid_attach(GTK_GRID(grid), button, 9, 0, 3, 1);
	button = 0;
	button = gtk_button_new_with_label("+");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_plus_min_j2), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 9, 1, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("-");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_minus_min_j2), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 9, 3, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("+");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_plus_second_j2), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 11, 1, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("-");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_minus_second_j2), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 11, 3, 1, 1);
	//bouton go
	button = 0;
	...
	gtk_grid_attach(GTK_GRID(grid), button, 6, 8, 6, 1);
	//creation des label
	game->label_custom[4] = gtk_label_new("00");
	context = gtk_widget_get_style_context(game->label_custom[4]);
	gtk_style_context_add_class(context, "mod_label");
	button = 0;
	button = gtk_label_new(":");
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_label");
	gtk_grid_attach(GTK_GRID(grid), game->label_custom[4], 6, 6, 1, 1);
	game->label_custom[5] = gtk_label_new("00");
	context = gtk_widget_get_style_context(game->label_custom[5]);
	gtk_style_context_add_class(context, "mod_label");
	gtk_grid_attach(GTK_GRID(grid), game->label_custom[5], 8, 6, 1, 1);
	gtk_grid_attach(GTK_GRID(grid), button, 7, 6, 1, 1);
	//temps bonus jouer premier
	button = 0;
	button = gtk_label_new("temps du bonus:");
	context = gtk_widget_get_style_context(button);
	...
	button = 0;
	button = gtk_button_new_with_label("+");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_plus_min_b1), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 6, 5, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("+");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_plus_second_b1), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 8, 5, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("-");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_minus_min_b1), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 6, 7, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("-");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_minus_second_b1), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 8, 7, 1, 1);
	//creation des label
	game->label_custom[6] = gtk_label_new("00");
	context = gtk_widget_get_style_context(game->label_custom[6]);
	gtk_style_context_add_class(context, "mod_label");
	button = 0;
	button = gtk_label_new(":");
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_label");
	gtk_grid_attach(GTK_GRID(grid), game->label_custom[6], 9, 6, 1, 1);
	game->label_custom[7] = gtk_label_new("00");
	context = gtk_widget_get_style_context(game->label_custom[7]);
	gtk_style_context_add_class(context, "mod_label");
	gtk_grid_attach(GTK_GRID(grid), game->label_custom[7], 11, 6, 1, 1);
	gtk_grid_attach(GTK_GRID(grid), button, 10, 6, 1, 1);
	//temps bonus jouer deux
	button = 0;
	button = gtk_label_new("temps du bonus:");
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_stitre2");
	gtk_label_set_xalign(GTK_LABEL(button), 0.6);
	gtk_grid_attach(GTK_GRID(grid), button, 9, 4, 3, 1);
	button = 0;
	button = gtk_button_new_with_label("+");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_plus_min_b2), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 9, 5, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("+");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_plus_second_b2), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 11, 5, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("-");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_minus_min_b2), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 9, 7, 1, 1);
	button = 0;
	button = gtk_button_new_with_label("-");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(ft_minus_second_b2), (gpointer)game);
	context = gtk_widget_get_style_context(button);
	gtk_style_context_add_class(context, "mod_button");
	gtk_grid_attach(GTK_GRID(grid), button, 11, 7, 1, 1);
	//button = 0;
	//button = gtk_label_new(" ");
//	context = gtk_widget_get_style_context(button);
