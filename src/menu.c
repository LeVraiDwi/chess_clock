...
void	ft_menu(s_game *game)
{
	...
	gtk_grid_attach(GTK_GRID(game->grid_custom), button, 1, 1, 1, 1);
	menu_item = gtk_button_new_with_label("2 minutes + 1 seconde;s");
	gtk_grid_attach(GTK_GRID(game->grid_custom), button, 1, 2, 1, 1);
	...
	//ajout custome
	/*menu_item = gtk_button_new_with_label("Custom");
	g_signal_connect(G_OBJECT(menu_item), "clicked", G_CALLBACK(ft_custom), (gpointer)game);
